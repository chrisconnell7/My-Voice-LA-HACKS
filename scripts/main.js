...
window.getUIString = (englishKey) => {
  ...
  const dict =
    window.staticTranslations[langCode] || window.staticTranslations["en-US"];

  // Return the translation if it exists, otherwise return the original English key
  return dict && dict.ui && dict.ui[englishKey]
    ? dict.ui[englishKey]
    : englishKey;
};
...
ngramWorker.onmessage = (e) => {
  if (e.data.status === "ready") {
    ...
    if (aiPlaceholder) {
      aiPlaceholder.textContent = window.getUIString(
...
function renderAIChips(predictions) {
  ...
  predictions.forEach((word) => {
    ...
    if (window.gemmaContextWords?.includes(word))
...
window.renderSuggestions = () => {
  ...
  if (!window.categoryData || !window.categoryData[window.currentCategory]) {
    grid.innerHTML =
      '<div style="padding: 20px; color: #666;">No phrases found for this category.</div>';
    return;
  }
...
document.addEventListener("mouseout", (e) => {
  const target = e.target.closest(
    "button, .ai-chip, .suggestion-btn, .category-btn, .key",
  );
...
document.addEventListener("mouseover", (e) => {
  const target = e.target.closest(
    "button, .ai-chip, .suggestion-btn, .category-btn, .key, .action-btn",
  );
  if (target) {
    isHovering = true;
...
window.toggleMenu = () => {
  ...
  document
    ...
    ?.classList.toggle("open", window.menuOpen);
};
...
window.renderLanguageList = (filter = "") => {
  ...
  const filtered = window.languages.filter(
    (l) =>
...
window.applyLanguage = () => {
  ...
  document.getElementById("menuLangName").textContent =
    window.currentLang.native;
  document.getElementById("menuLangSub").textContent =
  ...
  const dict =
    window.staticTranslations[window.currentLang.code] ||
    window.staticTranslations["en-US"];
...
window.renderCategories = () => {
  ...
  grid.innerHTML = window.categoryMeta
    .map((cat) => {
...
window.renderSuggestions = () => {
  ...
  if (
    !window.categoryData ||
    !window.categoryData[window.currentCategory] ||
    window.categoryData[window.currentCategory].length === 0
  ) {
    grid.innerHTML =
      '<div style="padding: 20px; color: #666;">No phrases found here.</div>';
    return;
  }
...
window.selectedVoiceId =
  localStorage.getItem("userVoiceId") || "pNInz6obpgDQGcFmaJgB";
...
function moveVirtualCursor(x, y) {
  ...
  // FIX: Added '[onclick]' to ensure items like .lang-item and .generated-item-text are caught
  const elementUnderGaze = document.elementFromPoint(x, y);
  const targetButton = elementUnderGaze
    ? elementUnderGaze.closest(
        "button, [onclick], .ai-chip, .suggestion-btn, .category-btn, .key, .action-btn",
      )
    : null;
  ...
  if (targetButton) {
    ...
    isHovering = false;
    currentTarget = null; // <-- CRITICAL: Clears memory so you can re-select the same button
    dwellLockout = false; // <-- CRITICAL: Unlocks the engine
  }
}
